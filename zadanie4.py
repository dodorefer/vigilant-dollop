...
# 6 нах в массиве макс и мин элементы, которые делятся на предшествующий элемент и имеют четное частное, если эл не найд сообщ вывод в консоль
...
# 7 нах мин и макс элементы в массиве кот имеют четную разницу с соседними элементами
result = []
...
# 8 находит в массиве элементов со значением 40, вычисляет сред знач элементов перед ним и вывод элемент на консоль
...
# 9 находит в массиве максим элемент кот делится на 10
max_num = 0
for num in arr:
...
# 10 находит перв элемент в массиве который имеет четную разницу с соседними элементами и делится на 3
for i in range(1, len(arr)):
...
# 11 вычисл сумму квад-в после каждого элемента в массиве и выводит первый элемент для кот среднее значение  квадратов меньше или равно
sum_sq = 0
count = 0
max_el = None
for i in range(len(arr)):
    ...
    if sum_sq / count <= 10:
        ...
        print("Не найдено")
# 12 вычисляет тангенс каждого элемента в массиве, нах макс значение тангенса, меняет местами элемент с макс массивом
max_tg = -math.inf
max_index = -1
for i in range(len(arr)):
    tg_x = math.tan(arr[i])
